...
public sealed class CurrencyDisplayNameStore
{
    ...
    public void Load(string dllDirectory)
    {
        ...
        {
            if (!TryExtractEmbeddedDefaults(dataFilePath))
            {
    ...
    public void TryRegisterDiscovery(
        string itemPath,
        string? priceId,
        string displayName)
    {
        ...
        {
            TryAppendPathNinjaId(fileName, priceId);
            byPathNinjaId[fileName] = priceId;
        }
    ...
    private static bool TryExtractEmbeddedDefaults(string path)
    {
        try
        {
            var assembly = typeof(CurrencyDisplayNameStore).Assembly;
            var resourceName = assembly
                .GetManifestResourceNames()
                .FirstOrDefault(name => name.EndsWith("currency-names.json", StringComparison.OrdinalIgnoreCase));
        ...
        {
            Log.Error($"Failed to extract embedded currency names: {ex.Message}", "Better Loot Tracker");
            return false;
        }
    }
